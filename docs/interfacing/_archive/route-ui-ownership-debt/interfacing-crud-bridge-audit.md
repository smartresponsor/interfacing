...
- The current slice already has a strong CRUD resource registry and explorer, but `InterfaceAbstractCrudResourceContributionService` generated `app_crud_index` and `app_crud_new` route names that were not declared in the slice.
...
- The shell needs a fast admin-provider-style compensation layer: index, new, show, edit and delete screens must open from the address bar using the same CRUD grammar, even before all owning components are connected.
