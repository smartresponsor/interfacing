...
function ExpandTemplate([string]$Value, [string]$Slug, [string]$Tag)
{
    if ($null -eq $Value)
    {
        return ""
...
function ShouldThrottle([string]$PackId, [string]$TimerIso)
{
    ...
    if ( [string]::IsNullOrWhiteSpace($triggerPack))
    {
    ...
    # Expand asset templates per consumer slug + resolved tag
    $zipName = ExpandTemplate $zipName $consumerSlug $tag
    $shaName = ExpandTemplate $shaName $consumerSlug $tag
...
{
    $b = if ( [string]::IsNullOrWhiteSpace($Branch))
    {
        "master"
    }
    else
    ...
    {
        $hdr["Authorization"] = "Bearer $ReadToken"
    }
    try
    {
...
function DownloadAssets([string]$Owner, [string]$Repo, [string]$Tag, [string]$ZipName, [string]$ShaName, [string]$ReadToken, [string]$Branch)
{
    ...
    $useBranch = if ( [string]::IsNullOrWhiteSpace($Branch))
    {
        "master"
    }
    else
    ...
    $dlKey = if ( [string]::IsNullOrWhiteSpace($Tag))
    {
        ("branch-" + $useBranch)
    }
    else
...
{
    $raw = (Get-Content -LiteralPath $ShaPath -Raw -Encoding UTF8).Trim()
    if ( [string]::IsNullOrWhiteSpace($raw))
    {
        throw "SHA256 file is empty: $ShaPath"
    }

    $zipBase = [System.IO.Path]::GetFileName($ZipPath)

    $expected = $null
    $lines = @($raw -split "`n")
    if ($lines.Count -gt 1 -or $raw -match "\s{2,}")
    {
        foreach ($ln in $lines)
        {
            $line = $ln.Trim()
            ...
            #   <hash>  <file>
            #   <hash> *<file>
            $m = [regex]::Match($line, "^(?<h>[a-fA-F0-9]{64})\s+[* ]?(?<f>.+)$")
            if ($m.Success)
            {
                $fileName = $m.Groups["f"].Value.Trim()
                if ($fileName -eq $zipBase)
                {
    ...
    {
        $expected = ($raw.Split(" ")[0]).ToLowerInvariant()
    }

    if (-not$expected)
    {
        throw "Unable to parse expected sha256 from: $ShaPath"
    }

    $actual = Sha256File $ZipPath
    if ($expected -ne $actual)
    {
        throw "SHA256 mismatch. expected=$expected actual=$actual (file=$zipBase)"
    }
    return $actual
}
...
$consumerSlug = ($env:GITHUB_REPOSITORY -split "/")[-1].Trim()
if ( [string]::IsNullOrWhiteSpace($consumerSlug))
{
    $consumerSlug = "repo"
}
...
{
    $baseBranch = $env:AUTOMATE_BASE_BRANCH
}
if ( [string]::IsNullOrWhiteSpace($baseBranch))
{
    $defBranch = Get-OptionalProp $defaults 'baseBranch'
    $baseBranch = $( if ($defBranch)
    {
        [string]$defBranch
    }
    else
...
{
    $readToken = [string]$env:AUTOMATER_SOURCE_TOKEN
}
if ( [string]::IsNullOrWhiteSpace($readToken))
{
...
{
    $globalTimer = $env:AUTOMATE_PUSH_TIMER
}
if ( [string]::IsNullOrWhiteSpace($globalTimer))
{
    $defTimer = Get-OptionalProp $defaults 'pushTimer'
    $globalTimer = $( if ($defTimer)
    {
        [string]$defTimer
    }
    else
...
foreach ($pack in $packs)
{
    ...
    if (-not$src)
    {
        throw "Pack $id missing source."
    }
    if (-not$apply)
    {
        throw "Pack $id missing apply."
    }
    ...
    if (-not$zipName)
    {
        $zipName = [string](Get-OptionalProp $defSrc 'assetZip')
    }
    if (-not$zipName)
    {
        $zipName = "automate-pack-{slug}-{tag}.zip"
    }

    $shaName = [string](Get-OptionalProp $src 'assetSha')
    if (-not$shaName)
    {
        $shaName = [string](Get-OptionalProp $defSrc 'assetSha')
    }
    if (-not$shaName)
    {
        $shaName = "SHA256SUMS"
    ...
    if (-not$topFolder)
    {
        $topFolder = [string](Get-OptionalProp $defSrc 'topFolder')
    }
    ...
    if (-not$srcBranch)
    {
        $srcBranch = [string](Get-OptionalProp $defSrc 'branch')
    }
    if (-not$srcBranch)
    {
        $srcBranch = $baseBranch
    }

    $targetRoot = [string](Get-OptionalProp $apply 'targetRoot')
    if (-not$targetRoot)
    {
        $targetRoot = [string](Get-OptionalProp $defApply 'targetRoot')
    }
    if ([string]::IsNullOrWhiteSpace($targetRoot) -or $targetRoot -eq "." -or $targetRoot -eq "./")
    {
        $targetRoot = "." + $consumerSlug
    }
