...
final readonly class InterfaceCrudExplorerViewBuilder implements InterfaceCrudExplorerViewBuilderInterface
{
    public function __construct(
        private InterfaceCrudResourceExplorerProviderInterface $crudResourceExplorerProvider,
    ...
    public function buildRouteExpectationsPayload(): array
    {
        ...
        return [
            ...
            'handoffRoutes' => $this->handoffRoutePayload(),
            'expectations' => $rows,
        ];
    }
    ...
    public function buildOperationsPayload(): array
    {
        ...
        foreach ($this->handoffRouteCatalog() as $operation => $definition) {
     ...
     */
    private function handoffRouteCatalog(): array
    {
    ...
    private function handoffRoutePayload(): array
    {
        $payload = [];
        foreach ($this->handoffRouteCatalog() as $operation => $definition) {
            $payload[] = [
    ...
    private function expectedPattern(object $resource, InterfaceCrudOperationGrammarInterface $definition): string
    {
        return $definition->expectedPattern($resource->resourcePath());
    }

    private function sampleUrl(object $resource, InterfaceCrudOperationGrammarInterface $definition): string
    {
