...
#[AsLiveComponent('interfacing_screen', template: 'live/screen.html.twig')]
final class InterfaceScreenLiveComponent
{
    #[LiveProp]
    ...
    public function __construct(
        private readonly InterfaceScreenRegistryInterface $screenRegistry,
        private readonly InterfaceActionDispatcherInterface $actionDispatcher,
    ) {
    }
    ...
    public function screen(): InterfaceScreenSpecInterface
    {
    ...
    public function invokeAction(string $actionId, array $payload = []): void
    {
        ...
        if (InterfaceActionResult::TYPE_OK === $result->type()) {
        ...
        if (InterfaceActionResult::TYPE_VALIDATION_ERROR === $result->type()) {
        ...
        if (InterfaceActionResult::TYPE_DOMAIN_ERROR === $result->type()) {
            $this->domainError = (string) ($result->payload()['message'] ?? 'Domain error.');

            return;
        }

        if (InterfaceActionResult::TYPE_RELOAD === $result->type()) {
            $this->state = $this->screen()->defaultState();

            return;
        }

        if (InterfaceActionResult::TYPE_REDIRECT === $result->type()) {
