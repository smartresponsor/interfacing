<?php

declare(strict_types=1);

namespace App\Interfacing\Presentation\Controller;

use App\Interfacing\CatalogInterface\InterfaceScreenSpecCatalogInterface;
use App\Interfacing\ProviderInterface\Shell\InterfaceShellNavProviderInterface;
use App\Interfacing\ResolverInterface\Access\InterfaceRoleAccessResolverInterface;
use App\Interfacing\ServiceInterface\Rendering\InterfaceRendererInterface;
use Symfony\Bundle\FrameworkBundle\Controller\AbstractController;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Attribute\Route;

final class InterfaceScreenController extends AbstractController
{
    public function __construct(
        private readonly InterfaceScreenSpecCatalogInterface $screenCatalog,
        private readonly InterfaceShellNavProviderInterface $navProvider,
        private readonly InterfaceRoleAccessResolverInterface $accessResolver,
        private readonly InterfaceRendererInterface $renderer,
    ) {
    }

    #[Route('/interfacing/catalog/screen/{screenId}', name: 'interfacing_catalog_screen_show', requirements: ['screenId' => '[a-z0-9][a-z0-9._-]{0,63}'], methods: ['GET'])]
    public function show(string $screenId): Response
    {
        $spec = $this->screenCatalog->get($screenId);
        if (!$this->accessResolver->canAccess($spec->requireRole())) {
            throw $this->createAccessDeniedException();
        }

        return $this->renderer->render('shell/catalog_screen.html.twig', [
            'title' => $spec->title(),
            'screenId' => $spec->id(),
            'navGroupList' => $this->navProvider->provide(),
            'activeScreenId' => $spec->id(),
            'activeViewId' => $spec->viewId(),
            'payload' => ['screen' => ['id' => $spec->id(), 'title' => $spec->title(), 'description' => $spec->description()]],
        ]);
    }
}
