...
namespace App\Interfacing\Store\Demo;
...
final class InterfaceDemoUserProfileStore implements InterfaceDemoUserProfileStoreInterface
{
    ...
    public function save(string $name, string $email): void
    {
        ...
        if (!is_dir($dir)) {
            ...
            if (!$created && !is_dir($dir)) {
                throw new InterfaceDomainOperationFailed('Failed to create profile directory.', ['dir' => $dir, 'error' => $mkdirError]);
            }
        }
        ...
        if (false === $ok) {
            throw new InterfaceDomainOperationFailed('Failed to write profile storage.', ['error' => $writeError]);
        }
    }
}
