...
final readonly class InterfaceEcommerceContractRegistryProvider implements InterfaceEcommerceContractRegistryProviderInterface
{
    public function __construct(private InterfaceEcommerceEvidenceRegistryProviderInterface $evidenceRegistryProvider)
    {
    }
    ...
    public function provide(): array
    {
        ...
        foreach ($this->evidenceRegistryProvider->provide() as $evidence) {
            if (!$evidence instanceof InterfaceEcommerceEvidenceItem) {
                continue;
            }

            $items[] = new InterfaceEcommerceContractItem(
        ...
        usort(
            $items,
            static fn (InterfaceEcommerceContractItem $left, InterfaceEcommerceContractItem $right): int => [
    ...
    /** @return list<string> */
    private function screenContracts(InterfaceEcommerceEvidenceItem $evidence): array
    {
    ...
    /** @return list<string> */
    private function dataContracts(InterfaceEcommerceEvidenceItem $evidence): array
    {
        return match ($evidence->evidenceGrade()) {
    ...
    /** @return list<string> */
    private function operationContracts(InterfaceEcommerceEvidenceItem $evidence): array
    {
        return match ($evidence->evidenceGrade()) {
    ...
    /** @return list<string> */
    private function policyContracts(InterfaceEcommerceEvidenceItem $evidence): array
    {
        return match ($evidence->evidenceGrade()) {
    ...
    /** @return list<string> */
    private function evidenceContracts(InterfaceEcommerceEvidenceItem $evidence): array
    {
        return match ($evidence->evidenceGrade()) {
            'complete' => ['audit events exist for create/update/delete', 'runtime smoke proof is current'],
            'partial' => ['audit event names and handoff smoke proof remain required'],
            default => ['route, data, operation, policy and audit evidence are required before promotion'],
        };
    }

    /** @return list<string> */
    private function openQuestions(InterfaceEcommerceEvidenceItem $evidence): array
    {
        return match ($evidence->evidenceGrade()) {
    ...
    private function note(InterfaceEcommerceEvidenceItem $evidence): string
    {
        return sprintf(
