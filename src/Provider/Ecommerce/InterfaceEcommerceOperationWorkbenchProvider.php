...
final readonly class InterfaceEcommerceOperationWorkbenchProvider implements InterfaceEcommerceOperationWorkbenchProviderInterface
{
    ...
    public function provide(): array
    {
        ...
        foreach ($this->screenCatalogProvider->provide() as $entry) {
            if (!$entry instanceof InterfaceEcommerceScreenEntry || !str_starts_with($entry->id(), 'crud.')) {
                continue;
            }
        ...
        foreach ($buckets as $key => $bucket) {
            /** @var array<string, InterfaceEcommerceScreenEntry> $action */
            $action = $bucket['action'];
            if (!isset($action['index'], $action['new'], $action['show'], $action['edit'], $action['delete'])) {
                continue;
            }

            $cards[] = new InterfaceEcommerceOperationCard(
                id: $key,
                zone: $bucket['zone'],
        ...
        usort(
            $cards,
            static fn (InterfaceEcommerceOperationCard $left, InterfaceEcommerceOperationCard $right): int => [
