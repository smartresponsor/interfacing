...
namespace App\Interfacing\Provider\Surface;
...
use App\Interfacing\ProviderInterface\Surface\InterfaceSurfaceAuditProviderInterface;

final readonly class InterfaceSurfaceAuditProvider implements InterfaceSurfaceAuditProviderInterface
{
    public function __construct(private InterfaceEcommerceScreenCatalogProviderInterface $screenCatalogProvider)
    {
    }
    ...
    private function shellItems(): array
    {
        return [
            ...
            new InterfaceSurfaceAuditItem('shell.operations', 'Shell coverage', 'Operation workbench', 'solid', '/interfacing/operations', 'Shell-native admin-provider-like operation surface generated from CRUD resources.'),
            new InterfaceSurfaceAuditItem('shell.admin-tables', 'Shell coverage', 'Admin tables', 'solid', '/interfacing/tables', 'Table-first admin surface for index-like CRUD review, empty states and route-transparent actions.'),
            new InterfaceSurfaceAuditItem('shell.crud-frames', 'Shell coverage', 'CRUD form frames', 'solid', '/interfacing/forms', 'Shell-native New/Edit/Delete operation frames generated from the canonical CRUD registry.'),
            new InterfaceSurfaceAuditItem('shell.crud-affordances', 'Shell coverage', 'CRUD affordances', 'solid', '/interfacing/affordances', 'Shell-native filter, search, sort, pagination and bulk-action affordances generated from the canonical CRUD registry.'),
            new InterfaceSurfaceAuditItem('shell.crud-readiness', 'Shell coverage', 'CRUD readiness', 'solid', '/interfacing/readiness', 'Shell-native readiness checklist showing which UI frames exist and which obligations remain component-owned.'),
            new InterfaceSurfaceAuditItem('shell.component-obligations', 'Shell coverage', 'Component obligations', 'solid', '/interfacing/obligations', 'Shell-native checklist proving component-owned fixtures, contracts, runtime handoff and evidence remain outside Interfacing.'),
            new InterfaceSurfaceAuditItem('shell.runtime-handoff', 'Shell coverage', 'Runtime handoff', 'solid', '/interfacing/runtime-handoff', 'Shell-native route/controller/query/command handoff catalog for planned and connected components.'),
            new InterfaceSurfaceAuditItem('shell.promotion-gates', 'Shell coverage', 'Promotion gates', 'solid', '/interfacing/promotions', 'Shell-native promotion checklist for moving planned/canonical resources toward connected runtime status.'),
            new InterfaceSurfaceAuditItem('shell.evidence-registry', 'Shell coverage', 'Evidence registry', 'solid', '/interfacing/evidence', 'Shell-native evidence registry for route, data, operation, policy and audit proof.'),
    ...
    {
        return [
            new InterfaceSurfaceAuditItem('crud.grammar.index', 'CRUD route grammar', 'Index route', 'solid', '/{resource}/', 'Generated from the canonical CRUD handoff resource path.'),
            new InterfaceSurfaceAuditItem('crud.grammar.new', 'CRUD route grammar', 'New route', 'solid', '/{resource}/new/', 'Create route follows the canonical CRUD handoff.'),
    ...
    private function componentCoverageItems(): array
    {
        ...
        foreach ($this->screenCatalogProvider->provide() as $entry) {
            if (!$entry instanceof InterfaceEcommerceScreenEntry || 'Interfacing' === $entry->component()) {
                continue;
            }
